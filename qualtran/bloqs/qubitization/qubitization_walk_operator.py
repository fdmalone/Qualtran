...
class QubitizationWalkOperator(SpecializedSingleQubitControlledGate):
    ...
    def build_composite_bloq(self, bb: 'BloqBuilder', **soqs: 'SoquetT') -> Dict[str, 'SoquetT']:
        ...
        reflect_regs = {reg.name: soqs[reg.name] for reg in self.reflect.signature}
        select_regs = {reg.name: soqs[reg.name] for reg in self.block_encoding.signature}
        if self.uncompute:
            reflect_regs |= bb.add_d(self.reflect.adjoint(), **reflect_regs)
            select_regs |= bb.add_d(self.block_encoding.adjoint(), **reflect_regs)
        else:
            select_regs |= bb.add_d(self.block_encoding, **select_regs)
            reflect_regs |= bb.add_d(self.reflect, **reflect_regs)
